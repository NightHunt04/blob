...
function ChatModel({modelName, modelDescription, modelImage, modelTitleColor, isImageGenerator, showcaseImages=null}) {
    ...
    return (
        <div className={`${lightTheme ? 'bg-[#e8e8e8] text-black' : 'bg-[#151515] text-gray-200'} relative w-full min-h-screen flex flex-col items-center justify-start overflow-hidden`}>
            ...
            {/* back button */}
            <div className={`fixed px-2 py-1 xl:px-4 xl:py-2 -left-[1500px] top-[20px] xl:left-[80px] xl:top-[60px] hover:cursor-pointer hover:opacity-80 rounded-lg ${lightTheme ? 'bg-[#ffffff] shadow-lg' : 'bg-[#333333]'}`} onClick={() => navigate(-1)}>
                <i className="fa-solid fa-arrow-left text-[16px]"></i>
            </div>

            {/* section for image generators */}
            <div className={`${hideDescription && isImageGenerator ? 'flex flex-col' : 'hidden'} pt-[100px] w-[90%] xl:w-9/12 items-center justify-center`}>
                
                {/* user's prompt along with user's google profile picture */}
                <div className="flex items-start justify-start w-[95%] xl:w-6/12 pb-7 gap-3">
                    <img src={profilePhotoURL} alt="profile" className="w-[20px] h-[20px] xl:w-[27px] xl:h-[27px] flex items-center justify-center rounded-full object-cover" />
                    <p className="font-inter text-[15px] xl:text-[19px]">{prevPrompt}</p>
                </div>

                {
                    imageURL === '' && isImageGenerator && !isServerError ? 
                    <div className="relative w-full flex flex-col items-center justify-center">
                        <img src="https://i.gifer.com/origin/3c/3c82e43002a5c632edebf76eadc6499a_w200.webp" alt="waiting" className="rounded-lg w-[90%] xl:w-[40%] object-cover"/>
                        <p className="absolute text-[14px] text-gray-400 xl:text-[17px] font-inter -bottom-8 xl:bottom-[30px]">Wait for a while, generating your image...</p>
                    </div>
                    :
                    <img src={imageURL} alt='try again' className={`${isImageGenerator ? 'flex' : 'hidden' } rounded-lg w-[95%] xl:w-[40%] object-cover`} />
                }

                {/* internal server indicator */}
                <div className={`${isServerError ? 'flex' : 'hidden'} rounded-lg p-4 flex-col items-center justify-center shadow-xl w-[95%] xl:w-[40%] ${lightTheme ? 'bg-white' : 'bg-[#282828]'}`}>
                    <p className="bg-red-600 font-inter rounded-lg px-2 py-1 xl:px-3 xl:py-2 font-medium text-white text-[13px] xl:text-[15px]">Internal server error!</p>
                    <img src="https://i.imgflip.com/f0pti.jpg" alt="internal server error" className="my-5 rounded-lg w-full object-cover"/>
                    <p className="font-inter text-[14px] w-full text-center xl:text-[17px] ">Please, try again by re-writing your prompt! Or if it's not responding then try again later.</p>
                </div>
                {/* re-generate button */}
                <button 
                    className={`${imageURL !== '' && isImageGenerator ? 'flex' : 'hidden'} gap-1 px-2 py-1 xl:px-3 xl:py-2 ${lightTheme ? 'bg-white' : 'bg-[#343434]'} items-center justify-center text-[13px] xl:text-[15px] font-inter font-medium border-[1px] border-gray-500 shadow-xl hover:opacity-80 mt-5 rounded-lg`} 
                    onClick={() => {
                        isRegenerate = true
                        handleRequest()
                    }}>
                        Re-generate 
                    <i className="fa-solid fa-arrows-rotate text-[11px] xl:text-[13px]"></i>
                </button>
                
                <div className={`${isImageGenerator ? 'fixed' : 'hidden'} px-3 py-2 bottom-[100px] rounded-lg mt-[140px] w-9/12 xl:w-[550px] ${lightTheme ? 'bg-[#ffffff]' : 'bg-[#343434]'}`}>
                    <p className="font-inter text-[14px] xl:text-[17px]"><span className="text-red-500 font-medium">Note:&nbsp;</span>You may need to turn off the ad-blockers for this one to get the images.</p>
                </div>
            </div> 

            {/* chat completion section */}
            <div className={`${hideDescription && !isImageGenerator ? 'flex flex-col' : 'hidden'} pt-[50px] xl:pt-[100px] pb-[90px] xl:pb-[150px] w-[93%] xl:w-9/12 max-h-[80%] overflow-hidden items-center justify-center`} >
                
                {
                    messages.map(message => {
                        return (
                            <div key={message.uuid} className={`${!message.isSentFromUser ? 'mb-[30px] xl:mb-[45px]' : ''} flex items-start justify-start w-[95%] xl:w-6/12 pb-4 xl:pb-7 gap-3`} >
                                <img src={message.profilePicture} alt="profile" className="w-[20px] h-[20px] xl:w-[27px] xl:h-[27px] flex items-center justify-center rounded-full object-cover" />
                                <p className={`font-inter text-[15px] w-[90%] xl:text-[19px] whitespace-pre-wrap`} dangerouslySetInnerHTML={{__html: message.text}}></p>
                            </div>
                        )
                    })
                }
            ...
            <div className={`${hideDescription ? 'hidden' : 'block'} font-inter mt-[50px] xl:mt-[100px] border-[1px] ${lightTheme ? 'bg-[#ffffff] border-gray-300' : 'bg-[#272727] border-gray-700'} shadow-xl flex flex-col items-center justify-center px-5 py-4 mb-[190px] xl:px-8 xl:py-7 w-9/12 xl:w-[700px] rounded-lg`}>
                <img src={modelImage} alt="Mistral" className={`${modelImage !== '0' ? 'flex' : 'hidden'} w-[50px] h-[50px] rounded-lg mb-2`}/>
                <p className={`font-semibold text-[18px] xl:text-[23px] py-2 text-left w-full ${modelTitleColor}`}>{modelName}</p>
                <p className={`text-[14px] w-full text-left xl:text-[17px] ${lightTheme ? 'text-gray-700' : 'text-gray-300'}`}>{modelDescription}</p>

                {
                    !isImageGenerator ? 
                    <div className="w-full">
                        <p className="pt-8 pb-3 text-[16px] xl:text-[18px] font-medium">Ask some questions from the given example below : </p>
                        <p className={`px-3 py-2 my-3 rounded-lg hover:cursor-pointer hover:opacity-85 text-[14px] w-full xl:text-[16px] ${lightTheme ? 'bg-[#e6e6e6]' : 'bg-[#3e3e3e]'}`} onClick={() => {
                    ...
                    :
                    <div className="w-full">
                        <p className="pt-8 pb-3 text-[16px] xl:text-[18px] font-medium">Some of the images generated by {modelName}:</p>
            ...
            <div className={`fixed bottom-5 border-[1px] rounded-lg flex items-center justify-center gap-3 px-2 py-2 w-[90%] xl:w-[720px] shadow-lg ${lightTheme ? 'bg-[#ffffff] border-gray-200' : 'bg-[#1b1b1b] border-gray-500'}`}>
                <input 
                    type="text" 
                    value={prompt} 
                    onChange={e => setPrompt(e.target.value)} 
                    className="w-full xl:w-[700px] xl:px-4 py-1 text-[15px] xl:text-[18px] font-inter bg-transparent outline-none" 
                ...
                <div className="flex items-center justify-center" onClick={() => {handleRequest()}}>
                    <i className="fa-solid fa-paper-plane px-3 xl:px-4 hover:opacity-60 hover:cursor-pointer"></i>
                </div>
            </div>
        </div>
    )
